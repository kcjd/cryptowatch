...
export const getTrending = async (vs_currency: string) => {
  const { data: trending } = await instance.get<TrendingResponse>('/search/trending')
...
export const getMarketChart = async (id: string, vs_currency: string, days: number) => {
  const { data } = await instance.get<MarketChartResponse>(`/coins/${id}/market_chart`, {
    params: {
      vs_currency,
      days
    }
